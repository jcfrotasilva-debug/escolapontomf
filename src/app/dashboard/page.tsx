...
function DashboardContent() {
  ...
  // Saldo de horas
  const [hoursBalance, setHoursBalance] = useState<any>(null);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-100">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        {hoursBalance && (
          <div className={`mb-6 rounded-3xl shadow-lg border-2 p-4 sm:p-5 ${
            hoursBalance.balanceType === 'superavit' ? 'bg-gradient-to-br from-green-50 to-emerald-50 border-green-300' :
            hoursBalance.balanceType === 'deficit' ? 'bg-gradient-to-br from-red-50 to-rose-50 border-red-300' :
            'bg-gradient-to-br from-slate-50 to-blue-50 border-slate-300'
          }`}>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg ${
                  hoursBalance.balanceType === 'superavit' ? 'bg-gradient-to-br from-green-500 to-emerald-600' :
                  hoursBalance.balanceType === 'deficit' ? 'bg-gradient-to-br from-red-500 to-rose-600' :
                ...
                <div>
                  <h3 className={`font-bold ${
                    hoursBalance.balanceType === 'superavit' ? 'text-green-900' :
                    hoursBalance.balanceType === 'deficit' ? 'text-red-900' :
                    'text-slate-900'
                  }`}>
                    Saldo de Horas do Mês
                  </h3>
                  <p className={`text-xs ${
                    hoursBalance.balanceType === 'superavit' ? 'text-green-700' :
                    hoursBalance.balanceType === 'deficit' ? 'text-red-700' :
                    'text-slate-600'
                  }`}>
                    {hoursBalance.workedDays} de {hoursBalance.workingDays} dias trabalhados
                  </p>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3 text-center">
                <div>
                  <p className="text-[10px] text-slate-500 uppercase font-medium">Esperado</p>
                  <p className="text-base sm:text-lg font-bold font-mono text-slate-700">
                    {Math.floor(hoursBalance.expectedMinutes / 60)}h{String(hoursBalance.expectedMinutes % 60).padStart(2, '0')}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] text-slate-500 uppercase font-medium">Efetivo</p>
                  <p className="text-base sm:text-lg font-bold font-mono text-slate-700">
                    {Math.floor(hoursBalance.actualMinutes / 60)}h{String(hoursBalance.actualMinutes % 60).padStart(2, '0')}
                  </p>
                </div>
                <div>
                  <p className={`text-[10px] uppercase font-medium ${
                    hoursBalance.balanceType === 'superavit' ? 'text-green-600' :
                    hoursBalance.balanceType === 'deficit' ? 'text-red-600' :
                    'text-slate-600'
                  }`}>Saldo</p>
