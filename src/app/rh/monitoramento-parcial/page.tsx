...
function PartialMonitoringContent() {
  ...
  const filteredServers = servers.filter((s) => {
    ...
    if (filter === 'partial') return s.partial && !s.complete;
    if (filter === 'none') return !s.partial;
    return true;
  });

  const totalServers = servers.length;
  const completeCount = servers.filter((s) => s.complete).length;
  const partialCount = servers.filter((s) => s.partial && !s.complete).length;
  const noneCount = servers.filter((s) => !s.partial).length;
              ...
              {filteredServers.map((server) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {/* Faltantes */}
                    {server.partial && server.missing.length > 0 && (
